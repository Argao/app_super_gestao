...
<form action="{{route('site.contato')}}" method="post">
    ...
    <input type="text" placeholder="Telefone" value="{{ old('telefone') }}" class={{$classe}} name="telefone"
           id="telefone">
    @if($errors->has('telefone'))
        <p class="erro">{{$errors->first('telefone')}}</p>
    @else
        <br>
    @endif
    ...
    @if($errors->has('motivo_contatos_id'))
        <p class="erro">{{$errors->first('motivo_contatos_id')}}</p>
    @else
        <br>
    @endif
    ...
    <textarea class={{$classe}} name="mensagem" id="mensagem">
        {{ (old('mensagem') != '') ? old('mensagem') : 'Preencha aqui a sua mensagem'}}
    </textarea>
    @if($errors->has('mensagem'))
        <p class="erro">{{$errors->first('mensagem')}}</p>
    @else
        <br>
    @endif
